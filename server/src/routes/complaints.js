...
function registerComplaintRoutes(app, db) {
  app.post("/complaints", requireAuth, async (req, res) => {
    ...
    const complaint = { ...payload, complaint_number: complaintNumber, citizen_profile_id: payload.citizen_profile_id ?? req.user.id, submitted_by: payload.submitted_by ?? req.user.id, status: "unassigned", created_at: now, updated_at: now };
    const result = await db.collection("complaints").insertOne(complaint);

    await db.collection("complaint_timeline").insertOne({ complaint_id: result.insertedId.toString(), actor_profile_id: req.user.id, old_status: null, new_status: "unassigned", note: "Complaint submitted", created_at: now });
  ...
  app.post("/complaints/:id/assign", requireAuth, async (req, res) => {
    ...
    await db.collection("complaints").updateOne(
      { _id: new ObjectId(id) },
      { $set: { assigned_department_id: assignedDepartmentId, priority, expected_resolution_at: expectedResolutionAt, status: "assigned", updated_at: new Date().toISOString() } }
    );
  ...
  app.post("/complaints/:id/status", requireAuth, async (req, res) => {
    ...
    await db.collection("complaint_timeline").insertOne({ complaint_id: id, actor_profile_id: req.user.id, old_status: existing.status || null, new_status: status, note: note || null, created_at: new Date().toISOString() });
    const updated = await db.collection("complaints").findOne({ _id: new ObjectId(id) });
    return res.json(toPublicDoc(updated));
  });
  ...
  app.post("/complaints/:id/reopen", requireAuth, async (req, res) => {
    ...
    await db.collection("complaints").updateOne(
      { _id: new ObjectId(id) },
      { $set: { status: "reopened", reopened_at: new Date().toISOString(), updated_at: new Date().toISOString() } }
    );

    await db.collection("complaint_timeline").insertOne({ complaint_id: id, actor_profile_id: req.user.id, old_status: "resolved", new_status: "reopened", note: "Citizen reopened complaint", created_at: new Date().toISOString() });
    const updated = await db.collection("complaints").findOne({ _id: new ObjectId(id) });
    return res.json(toPublicDoc(updated));
  });
}
