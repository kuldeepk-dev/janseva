...
type OperatorTab = "home" | "assign" | "profile";

export function OperatorBottomLayout({ activeTab }: { activeTab: OperatorTab }) {
  const router = useRouter();
  const [layoutWidth, setLayoutWidth] = useState(0);
  const tabIndex = activeTab === "home" ? 0 : activeTab === "assign" ? 1 : 2;
  ...
  return (
    ...
    >
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          if (activeTab !== "assign") {
            router.push("/operator/assign" as never);
          }
        }}
      >
        <MaterialIcons
          name="assignment"
          size={20}
          color={activeTab === "assign" ? "#00714D" : "#444651"}
        />
        <Text style={activeTab === "assign" ? styles.activeText : styles.tabText}>
          Assign
        </Text>
      </TouchableOpacity>
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          if (activeTab !== "profile") {
            router.push("/operator/profile" as never);
          }
        }}
      >
        <MaterialIcons
